...
if (document.URL.includes("rarbg")) {
   movieList = document.getElementsByClassName("lista2");
    ...
    movieList = document.querySelector('#mcol > div > div.block-content > div > div > div > table > tbody').getElementsByTagName("tr");
    [].slice.call(movieList, 3,movieList.length-1);
}

for (i in movieList) {
    var movie = movieList[i].querySelector('td:nth-child(2) > a:nth-child(1)').getAttribute("onmouseover");
    ...
    movieList[i].querySelector('td:nth-child(1) > a > img').setAttribute("src", url);
    movieList[i].querySelector('td:nth-child(1) > a > img').setAttribute("height",height);
    movieList[i].querySelector('td:nth-child(1) > a > img').setAttribute("width", width);
}
